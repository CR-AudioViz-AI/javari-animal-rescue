...
export function getActions(): ToolAction[] {
  return [
    { id: 'adoption_listing',  label: '🐕 Adoption Listing',   desc: 'Write a compelling adoption profile',       buildPrompt: function(v: Record<string, string>) { return 'Write a heartfelt, detailed adoption listing for: ' + (v.animalName || 'the animal') + ', ' + (v.breed || '') + ', ' + (v.age || '') + '. Personality: ' + (v.personality || '') + '. Include story, temperament, ideal home, and a compelling call-to-action.' } },
    { id: 'grant_application', label: '📋 Grant Application',   desc: 'Write a full grant application',            buildPrompt: function(v: Record<string, string>) { return 'Write a professional grant application for ' + (v.orgName || 'our animal rescue') + ' requesting $' + (v.amount || '5,000') + ' for: ' + (v.purpose || 'rescue operations') + '. Include mission statement, impact metrics, budget justification, and a persuasive closing ask.' } },
    { id: 'fundraising_email', label: '💌 Fundraising Email',   desc: 'Compelling donor email that converts',      buildPrompt: function(v: Record<string, string>) { return 'Write a compelling fundraising email for ' + (v.orgName || 'our animal rescue') + '. Campaign: ' + (v.campaign || 'general operations') + '. Goal: $' + (v.goal || '1,000') + '. Make it emotional, urgent, and specific with a clear call-to-action.' } },
    { id: 'social_media_post', label: '📱 Social Media Post',   desc: 'Attention-grabbing post for all platforms', buildPrompt: function(v: Record<string, string>) { return 'Create 3 social media posts (Instagram, Facebook, X/Twitter) for ' + (v.orgName || 'our rescue') + ' about: ' + (v.topic || v.animalName || 'an animal in need') + '. Make each platform-specific with hashtags and emojis.' } },
    { id: 'donation_appeal',   label: '🙏 Donation Appeal',     desc: 'Emergency or general donation appeal',      buildPrompt: function(v: Record<string, string>) { return 'Write a powerful donation appeal letter for ' + (v.orgName || 'our rescue') + '. Situation: ' + (v.situation || 'animals need help urgently') + '. Be specific, emotional, and include impact per dollar donated.' } },
    { id: 'volunteer_guide',   label: '🤝 Volunteer Guide',     desc: 'Onboarding guide for new volunteers',       buildPrompt: function(v: Record<string, string>) { return 'Create a complete volunteer onboarding guide for ' + (v.orgName || 'our animal rescue') + '. Include: welcome message, shelter rules, role descriptions, animal handling basics, shift scheduling, and emergency contacts template.' } },
    { id: 'care_guide',        label: '🏥 Animal Care Guide',   desc: 'Detailed care instructions for a pet',      buildPrompt: function(v: Record<string, string>) { return 'Write a complete care guide for ' + (v.animalName || 'this animal') + ' — ' + (v.breed || v.species || 'mixed breed') + '. Include: feeding, exercise, grooming, vet care, behavioral notes, and special needs: ' + (v.specialNeeds || 'none noted') + '.' } },
  ]
}
